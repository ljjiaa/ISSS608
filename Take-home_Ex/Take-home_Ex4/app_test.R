...
# Define sub tabs needed for "Time Series Analysis and Forecasting" tabName "Univariate"
TimeSeriesSubTabs <- tabsetPanel(
...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName = "Univariate",
            h2("Time Series Analysis and Forecasting"), 
            TimeSeriesSubTabs
...
server <- function(input, output) {
  ...
  output$STL <- renderPlot({
    # Assuming 'weather', 'start_date', 'end_date', and 'single_station' are defined elsewhere in your app
    ...
    # Ensure that 'filtered_period_sstn' contains a time series column, here assuming 'Mean Temperature (°C)'
    # Decompose the time series using STL after extracting the target column
    # Adjust parameters as needed for your specific case
    # Note: You may need to convert the filtered data into a ts object if 'Mean Temperature (°C)' is not in the correct format
    stl_decomp <- stl(filtered_period_sstn[[`Mean Temperature (°C)`]], s.window = "periodic")
    
    # Plotting the decomposition
    plot <- autoplot(stl_decomp) + 
      ggtitle("STL Decomposition of Mean Temperature") + 
      theme_minimal() +
      theme(plot.background = element_rect(fill = "#edf2f7"))
    
    # Convert to an interactive plot
    ggplotly(plot) %>% layout(width = 700, height = 700, plot_bgcolor="#edf2f7")
  })
